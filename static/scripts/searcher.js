    ...
    .then(data => {
        data.forEach(function(knife) {
            $('#valuetable tr:last').after(
                `<tr><th><img width=96 height=96 src="images/${knife['NAME'].replace(' ', '_')}.png"></th><th>${knife['NAME']}</th><th>${knife['DEMAND']}</th><th>${knife['VALUE']}</th><th>${knife['OBTAIN']}</th><th>${knife['ORIGIN']}</th></tr>`
            );
        })
    });
}
